...
- [TABLE_OF_CONTENTS.md](TABLE_OF_CONTENTS.md) - concise repository index
- [ARCHITECTURE.md](ARCHITECTURE.md) - structural overview of packages, layers, and relationships
- [`docs/PHASE_1.md`](docs/PHASE_1.md) and [`docs/PHASE_2.md`](docs/PHASE_2.md) - contextual planning references for the current package layout and broader future direction
...
- **Sidebar:** Choose environment (Open World or Maze) from the dropdown; check one or more agents (for example, RL Base Agent or RL with Fractal Exploration). Set episodes, max steps, and hyperparameters (alpha, gamma, epsilon). Use "Reset Agents" to clear cached Q-tables; use "Save / Load" to download or upload a learned Q-table (`.npz`) after training.
...
This interface is documented rather than enforced to keep the code lightweight and readable for this assignment. In a larger system, such as the planned RC UGV control project, this contract will be formalized using a shared `types.py` module or Python `Protocol`s once multiple runners, agents, or environments introduce pressure for stronger static guarantees.

The training loop (`train_runner.py`) is intentionally decoupled from specific environment and agent implementations.

Rather than requiring concrete base classes, the runner relies on minimal **behavioral contracts**. These contracts are documented here to clarify system boundaries and support future extensions, without introducing unnecessary abstraction for this assignment. [`ARCHITECTURE.md`](ARCHITECTURE.md) and [`TABLE_OF_CONTENTS.md`](TABLE_OF_CONTENTS.md) provide the repository-level context around that core contract.
